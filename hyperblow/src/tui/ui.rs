...
fn drawTabsSection<B: Backend,>(frame: &mut Frame<B,>, area: Rect, state: Rc<TUIState,>,) {
    ...
    match *state.tab.borrow() {
        Tab::Details => DetailsTab::draw(frame, area, state.clone(),),
        Tab::Bandwidth => BandwidthTab::draw(frame, area, state.clone(),),
        Tab::Files => FilesTab::draw(frame, area, state.clone(),),
        Tab::Trackers => TrackersTab::draw(frame, chunks[1], state.clone(),),
        Tab::Peers => PeersTab::draw(frame, area, state.clone(),),
        Tab::Pieces => PiecesTab::draw(frame, area, state.clone(),),
        Tab::None => {
            // Draws nothing
        }
    };
